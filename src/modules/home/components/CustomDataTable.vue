                        ...
                        <tr v-for="(row, rowIndex) in filteredItems" :key="rowIndex"
                            ...
                            <th v-for="(cell, cellIndex) in Object.values(row ? row : [])" :key="cellIndex" scope="row"
                                ...
                                <div v-show="columns[cellIndex].isShowing">
                                    <div v-if="columns[cellIndex].head === 'estadoId'">
                                        <button v-if="cell === '1'" @click="emitRowId(row)" type="button"
                                    ...
                                    <div v-else-if="columns[cellIndex].head === 'imagen'">
                                        <img :src="`${url}${cell}`" alt="imagen" class="w-50 h-50 rounded-xl">
                                    </div>
                                    <div v-else>
                                        {{ cell }}
                                    </div>
...
const emitRowId = (id: any) => {
    const emitId = Object.values(id).map((item) => item)
